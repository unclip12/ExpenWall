...
import { Save, CheckCircle, AlertCircle, Globe, ChevronDown, Check, Search, Key, ExternalLink, Loader2, Wallet, Plus, Trash2, CreditCard, Banknote, Smartphone } from 'lucide-react';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ currentApiKey = '', onApiKeyUpdate, userId }) => {
  ...
  const handleDeleteWallet = async (id: string) => {
      if(confirm('Delete this wallet? Transactions linked to it will remain but lose the link.')) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="space-y-4">
                  ...
                  <form onSubmit={handleAddWallet} className="space-y-3">
                      ...
                      <div className="flex gap-2">
                          {['bank', 'cash', 'credit', 'digital'].map((type) => (
                              <button
                                type="button"
                                key={type}
                                onClick={() => setNewWalletType(type as any)}
                      ...
                      >
                          {isAddingWallet ? <Loader2 className="w-5 h-5 animate-spin"/> : <span className="flex items-center"><Plus className="w-5 h-5 mr-2"/>Create Wallet</span>}
                      </button>
                  </form>
              </div>
      ...
      <div className="bg-white/70 backdrop-blur-xl p-6 rounded-3xl shadow-xl border border-white/20">
        ...
        <div className="max-w-md relative" ref={dropdownRef}>
               ...
               {isCurrencyListOpen && (
                 <div className="absolute top-full left-0 right-0 mt-2 bg-white/90 backdrop-blur-xl border border-slate-200 rounded-xl shadow-2xl z-50 overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-100">
      ...
      <div className="bg-white/70 backdrop-blur-xl p-6 rounded-3xl shadow-xl border border-white/20">
        ...
        <div className="max-w-2xl space-y-4">
           <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-xl backdrop-blur-sm">
              <a 
