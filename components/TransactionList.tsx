...
import { Search, Filter, Calendar, Edit2, Check, X, Sparkles, Loader2, ArrowRight, Trash2 } from 'lucide-react';
import { CURRENCIES, DEFAULT_CURRENCY, CATEGORIES } from '../constants';
import { updateTransactionInDb, deleteTransactionFromDb, addMerchantRule } from '../services/firestoreService';
...
export const TransactionList: React.FC<TransactionListProps> = ({ transactions, rules, userId, wallets = [] }) => {
  ...
  const getWalletName = (id?: string): string | null => {
    if (!id) return null;
    return wallets.find(w => w.id === id)?.name ?? null;
  };

  const handleEditClick = (t: { id: string; merchant: string; amount: number; date: string; category: Category; type: TransactionType; walletId?: string }) => {
    setEditingId(t.id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editForm.merchant !== originalMerchant) {
                ...
                {showRulePrompt ? (
                  <div className="flex flex-col space-y-3 p-2">
                    ...
                    <p className="text-sm text-slate-600 bg-white/50 p-3 rounded-lg border border-white/40">
                      Auto-rename <b>"{originalMerchant}"</b> <ArrowRight className="inline w-3 h-3" /> <b>"{editForm.merchant}"</b> in the future?
                    </p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-2 bg-slate-50 hover:bg-indigo-50 text-slate-300 hover:text-indigo-600 rounded-xl transition-all"
                      title="Edit"
                    >
                    ...
                    <button
                      ...
                      className="p-2 bg-slate-50 hover:bg-red-50 text-slate-300 hover:text-red-500 rounded-xl transition-all disabled:opacity-50"
                      title="Delete"
                    >
