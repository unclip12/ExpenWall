      ...
      {filteredProducts.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-slate-200 dark:border-slate-700 text-center animate-in fade-in duration-500">
           ...
           <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">
             {searchTerm ? 'No products found' : 'Product Catalog Empty'}
           </h3>
           <p className="text-slate-500 dark:text-slate-400 max-w-md mb-6">
             {searchTerm 
               ? `We couldn't find any products matching "${searchTerm}". Try a different search.` 
               : "Add transactions with detailed items (e.g. Milk, Bread) to automatically build your product price history database."}
           </p>
        </div>
      )}
    </div>
